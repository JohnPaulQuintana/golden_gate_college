            ...
            <div class="overflow-hidden shadow-sm rounded p-2">
                ...
                <form action="{{ route('dean.semester.manage.update') }}" method="post">
                    ...
                    <div class="flex flex-col max-w-full">
                        ...
                        <div class="mb-2">
                            <div class="flex max-w-full gap-2 mb-2">
                                ...
                                <div class="flex-1 space-y-3"> 
                                    <div class="relative">
                                        <select name="end_year" class="peer py-3 pe-0 ps-8 block w-full bg-white rounded-md border-t-transparent border-b-2 border-x-transparent border-b-gray-200 text-sm focus:border-t-transparent focus:border-x-transparent focus:border-b-[#32620e] focus:ring-0 disabled:opacity-50 disabled:pointer-events-none dark:border-b-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600 dark:focus:border-b-neutral-600">
                                         
                                            @for ($year = 2024; $year <= 2050; $year++)
                                                <option value="{{ $year }}" @if ((int)$endYear === $year) selected @endif>{{ $year }}</option>
                                            @endfor
                                        </select>
                                        <div class="absolute inset-y-0 start-0 flex items-center pointer-events-none ps-2 peer-disabled:opacity-50 peer-disabled:pointer-events-none">
                                            <svg class="shrink-0 size-4 text-green dark:text-neutral-500" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 448 512" fill="currentColor" stroke="currentColor"><!--!Font Awesome Free 6.6.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M224 256A128 128 0 1 0 224 0a128 128 0 1 0 0 256zm-45.7 48C79.8 304 0 383.8 0 482.3C0 498.7 13.3 512 29.7 512l388.6 0c16.4 0 29.7-13.3 29.7-29.7C448 383.8 368.2 304 269.7 304l-91.4 0z"/></svg>
                                ...
                                <div class="flex-1 space-y-3"> 
                                    <div class="relative">
                                        <select name="semester" class="peer py-3 pe-0 ps-8 block w-full bg-white rounded-md border-t-transparent border-b-2 border-x-transparent border-b-gray-200 text-sm focus:border-t-transparent focus:border-x-transparent focus:border-b-[#32620e] focus:ring-0 disabled:opacity-50 disabled:pointer-events-none dark:border-b-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600 dark:focus:border-b-neutral-600">
                                            ...
                                            <option value="first" @if ($semesterWithAcadYear->name === 'first') selected @endif>First Semester</option>
                                            <option value="second" @if ($semesterWithAcadYear->name === 'second') selected @endif>Second Semester</option>
                                            <option value="summer" @if ($semesterWithAcadYear->name === 'summer') selected @endif>Summer</option>
                                        </select>
                                        <div class="absolute inset-y-0 start-0 flex items-center pointer-events-none ps-2 peer-disabled:opacity-50 peer-disabled:pointer-events-none">
                                            <svg class="shrink-0 size-4 text-green dark:text-neutral-500" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 448 512" fill="currentColor" stroke="currentColor"><!--!Font Awesome Free 6.6.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M224 256A128 128 0 1 0 224 0a128 128 0 1 0 0 256zm-45.7 48C79.8 304 0 383.8 0 482.3C0 498.7 13.3 512 29.7 512l388.6 0c16.4 0 29.7-13.3 29.7-29.7C448 383.8 368.2 304 269.7 304l-91.4 0z"/></svg>
                                        </div>
                                ...
                                <div class="flex-1 space-y-3"> 
                                    <div class="relative">
                                        <select name="program" class="peer py-3 pe-0 ps-8 block w-full bg-white rounded-md border-t-transparent border-b-2 border-x-transparent border-b-gray-200 text-sm focus:border-t-transparent focus:border-x-transparent focus:border-b-[#32620e] focus:ring-0 disabled:opacity-50 disabled:pointer-events-none dark:border-b-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600 dark:focus:border-b-neutral-600">
                                            <option value="" disabled selected>Select program</option>
                                            
                                            @foreach ($programs as $item)
                                                @if ($item->is_open !== 'closed')
                                                    <option value="{{ $item->abbrev }}" @if($item->abbrev === $semesterWithAcadYear->abbrev) selected @endif>{{ $item->program }}</option>
                                                @endif
                                ...
                                <div class="flex-1 space-y-3"> 
                                    <div class="relative">
                                        <select name="status" class="peer py-3 pe-0 ps-8 block w-full bg-white rounded-md border-t-transparent border-b-2 border-x-transparent border-b-gray-200 text-sm focus:border-t-transparent focus:border-x-transparent focus:border-b-[#32620e] focus:ring-0 disabled:opacity-50 disabled:pointer-events-none dark:border-b-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600 dark:focus:border-b-neutral-600">
                                            <option value="active" @if($semesterWithAcadYear->status === 'active') selected @endif>Active</option>
                                            <option value="inactive" @if($semesterWithAcadYear->status === 'inactive') selected @endif>Inactive</option>
                    ...
                    <button type="button" class="bg-red-500 p-1 text-white rounded-md hover:bg-red-500">
                        <a href="{{ route('dean.academic') }}" class="bg-red-500 p-1 text-white rounded-md">Back to academic</a>
                    </button>
                    <button type="submit" class="bg-sidebar p-1 text-white rounded-md hover:bg-green">Update Semester</button>
                    <button type="button" class="bg-red-500 p-1 text-white rounded-md hover:bg-red-500">
                        <a href="{{ route('dean.semester.manage.delete', $semesterWithAcadYear->id) }}" class="bg-red-500 p-1 text-white rounded-md">Delete</a>
                    </button>
                </form>
