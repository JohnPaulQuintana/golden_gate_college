    ...
    @foreach ($programs as $item)
        <div class="p-4 bg-white border border-gray-200 rounded-lg dark:border-neutral-700">
            <div class="flex gap-2 items-center mb-2 border-b-2">
                <svg class="shrink-0 size-6 text-green" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"
            ...
            <div class="mb-2">
                <h3 class="mb-1 text-sm font-bold text-gray-600 dark:text-neutral-400">
                    Academic Year : {{ $item->semester->academic_year->academic_year ?? 'N/A' }} / <span
                        class="text-red-500 px-1">{{ $item->is_open }}</span>
                </h3>
            ...
            <div>
                <a href="{{ route('registrar.enroll', $item->id) }}"
                    class="bg-red-500 p-1 text-white rounded-md transition duration-300 ease-in-out hover:scale-105 hover:bg-red-700">Enroll
                    Student</a>
            </div>
        </div>
    @endforeach
